...
class AudibleClient:
    ...
    def from_file(
        ...
        if not auth_path.exists():
            raise AudibleAuthError(
                ...
                "Run initial authentication first."
            )
        
        try:
            auth = Authenticator.from_file(str(auth_path))
        except Exception as e:
            raise AudibleAuthError(f"Failed to load auth from {auth_file}: {e}")
    ...
    def from_login(
        ...
        cvf_callback: Optional[Callable] = None,
    ...
    def get_catalog_product(
        ...
        response_groups: Optional[str] = None,
        use_cache: bool = True,
    ) -> Optional[AudibleCatalogProduct]:
        """
        ...
        try:
            ...
            product = AudibleCatalogProduct.model_validate(
                response.get("product", response)
            )
    ...
    def search_catalog(
        ...
        # Cache results
        if self._cache:
            self._cache.set("search", cache_key, [p.model_dump() for p in products], ttl_seconds=self._cache_ttl_seconds)
    ...
    def get_listening_stats(
        self,
        use_cache: bool = True,
    ) -> Optional[AudibleListeningStats]:
        """
    ...
    def get_wishlist(
        ...
        # Cache results
        if self._cache:
            self._cache.set("library", cache_key, [p.model_dump() for p in products], ttl_seconds=self._cache_ttl_seconds)
