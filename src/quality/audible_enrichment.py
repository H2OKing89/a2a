...
class PricingInfo(BaseModel):
    ...
    list_price: Optional[float] = Field(default=None, description="Regular list price in USD")
    sale_price: Optional[float] = Field(default=None, description="Current sale/member price")
    credit_price: float = Field(default=1.0, description="Price in credits")
    currency: str = Field(default="USD", description="Currency code")
    price_type: Optional[str] = Field(default=None, description="Price type: sale, member, list")
...
class PlusCatalogInfo(BaseModel):
    ...
    plan_name: Optional[str] = Field(default=None, description="Plan name (US Minerva = Plus)")
    expiration_date: Optional[datetime] = Field(default=None, description="When removed from Plus")
    ...
    @property
    def expiration_display(self) -> Optional[str]:
...
class AudibleEnrichment(BaseModel):
    ...
    title: Optional[str] = Field(default=None, description="Title from Audible")
    
    # Ownership
    owned: bool = Field(default=False, description="User owns this on Audible")
    origin_type: Optional[str] = Field(default=None, description="How acquired (Purchase, Ayce, etc)")
    
    # Pricing
    pricing: Optional[PricingInfo] = Field(default=None, description="Pricing info")
    ...
    best_bitrate: Optional[int] = Field(default=None, description="Best available bitrate (kbps)")
...
class AudibleEnrichmentService:
    ...
    def __init__(
        ...
        self._library_asins: Optional[set[str]] = None
    ...
    def _parse_pricing(self, price_data: Optional[dict]) -> Optional[PricingInfo]:
    ...
    def enrich_single(self, asin: str, use_cache: bool = True) -> Optional[AudibleEnrichment]:
        """
        ...
        if product_images:
            ...
            enrichment.cover_image_url = (
                product_images.get("500") or 
                product_images.get("1024") or
                product_images.get("252") or
                next(iter(product_images.values()), None)
            )
